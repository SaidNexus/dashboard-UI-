    ...
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
      {stats.map((stats, index) => (
        ...
        >
          ...
          {/* Progressbar */}
          <div className="mt-4 h-2 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden duration-300">
            <div
              style={{
                width: stats.trend === "up" ? "75%" : "45%",
              }}
              className={`w-full h-full bg-gradient-to-r ${stats.color} rounded-full transition-all duration-300`}
            ></div>
          </div>
        </div>
      ))}
